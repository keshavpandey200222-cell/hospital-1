    ...
    <DashboardLayout role="patient" title="Overview">
      ...
      <main className="py-6 animate-fade-in-up">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold text-[#313D5A] dark:text-white mb-2">Good morning, John 👋</h1>
            <p className="theme-text-muted font-medium">Your health overview is looking great today.</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <div className="glass-card rounded-2xl p-6 relative overflow-hidden group cursor-pointer hover:border-primary-500/30" onClick={() => setShowCalendar(!showCalendar)}>
            ...
            <p className="text-sm theme-text-muted font-bold mb-1">Upcoming Visits</p>
            <p className="text-3xl font-bold theme-text-primary">{myAppointments.length}</p>
          </div>
          ...
          <div className="glass-card rounded-2xl p-6 relative overflow-hidden group cursor-pointer hover:border-secondary/30" onClick={() => setShowPrescriptions(!showPrescriptions)}>
            ...
            <p className="text-sm theme-text-muted font-bold mb-1">Active Prescriptions</p>
            <p className="text-3xl font-bold theme-text-primary">{prescriptions.length}</p>
          </div>
          ...
          <div className="glass-card rounded-2xl p-6 relative overflow-hidden group cursor-pointer hover:border-purple-500/30" onClick={() => setShowTests(!showTests)}>
            ...
            <p className="text-sm theme-text-muted font-bold mb-1">Test Results</p>
            <div className="flex items-end gap-2">
               <p className="text-3xl font-bold theme-text-primary">{testResults.length}</p>
          ...
          <div className="glass-card rounded-2xl p-6 relative overflow-hidden group flex flex-col justify-center items-center cursor-pointer hover:border-primary-500/50" onClick={() => setShowUploadModal(true)}>
            <div className="w-12 h-12 rounded-full border-2 border-dashed dark:border-slate-500 border-slate-300 theme-text-muted flex items-center justify-center text-xl mb-2 group-hover:text-primary-400 group-hover:border-primary-400 transition-colors">+</div>
            <p className="text-sm theme-text-muted font-bold">Upload Document</p>
            <p className="text-xs theme-text-muted opacity-60 mt-1 font-medium">{documents.length} files</p>
          </div>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          {/* Main Dose Alert Tracking */}
          <div className="lg:col-span-1">
            <MedicationTracker />
          </div>

          {/* Health Insights / Secondary Card can go here */}
          <div className="lg:col-span-1 glass-card rounded-2xl p-6 border-white/10 bg-gradient-to-br from-emerald-500/5 to-transparent">
             <h2 className="text-xl font-bold theme-text-primary mb-2">🌿 Health Insights</h2>
             <p className="theme-text-muted text-sm mb-4 font-medium">Your activity levels are up 12% this week! Keep it up for a healthier heart.</p>
             <div className="w-full h-2 dark:bg-white/5 bg-slate-200 rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500 w-[72%] shadow-[0_0_10px_rgba(16,185,129,0.4)]"></div>
             </div>
             <p className="text-right text-[10px] theme-text-muted mt-2 uppercase tracking-widest font-bold">Step Goal: 7,200 / 10,000</p>
          </div>
        </div>
        ...
        {showPrescriptions && (
          <div className="glass-card rounded-2xl p-6 mb-8 animate-fade-in-up">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold theme-text-primary">💊 Active Prescriptions</h2>
              <button onClick={() => setShowPrescriptions(false)} className="theme-text-muted hover:theme-text-primary text-sm font-bold transition-colors">Close ✕</button>
            </div>
            <div className="space-y-3">
              {prescriptions.map(rx => (
                <div key={rx.id} className="flex justify-between items-center p-4 dark:bg-white/5 bg-[#CBC5EA]/10 border dark:border-white/5 border-[#73628A]/10 rounded-xl">
                  <div>
                    <h3 className="theme-text-primary font-bold">{rx.name} <span className="theme-text-muted font-normal text-sm">({rx.dosage})</span></h3>
                    <p className="text-xs theme-text-muted mt-1 font-medium">{rx.frequency} • Prescribed by {rx.prescribedBy}</p>
                  </div>
        ...
        {showTests && (
          <div className="glass-card rounded-2xl p-6 mb-8 animate-fade-in-up">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold theme-text-primary">📄 Test Results</h2>
              <button onClick={() => setShowTests(false)} className="theme-text-muted hover:theme-text-primary text-sm font-bold transition-colors">Close ✕</button>
            </div>
            <div className="space-y-3">
              {testResults.map(test => (
                <div key={test.id} className={`p-4 dark:bg-white/5 bg-[#CBC5EA]/5 rounded-xl border ${!test.read ? 'border-primary-500/30 bg-primary-500/5' : 'dark:border-white/5 border-[#73628A]/10'}`}>
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="theme-text-primary font-bold flex items-center gap-2">
                        {test.testName}
                        {!test.read && <span className="text-[10px] bg-primary-500/20 text-primary-400 px-1.5 py-0.5 rounded">NEW</span>}
                      </h3>
                      <p className="text-xs theme-text-muted mt-1 font-medium">{test.date} • {test.doctor}</p>
                      <p className="text-sm theme-text-muted opacity-80 mt-2 font-medium">{test.summary}</p>
                    </div>
        ...
        <div className="flex items-center justify-between mb-6">
           <h2 className="text-xl font-bold theme-text-primary">Upcoming Appointments</h2>
          ...
          <div className="divide-y divide-white/5">
            {myAppointments.length === 0 ? (
              <div className="p-12 text-center theme-text-muted">
                <p className="text-lg mb-2 font-bold">No upcoming appointments</p>
                <Link href="/dashboard/patient/appointments" className="text-primary-400 hover:theme-text-primary transition-colors font-bold">Book one now →</Link>
              </div>
            ) : (
              myAppointments.map(appt => (
                <div key={appt.id} className="p-6 hover:dark:bg-white/[0.02] hover:bg-[#CBC5EA]/5 transition-colors flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                  <div className="flex gap-4 items-center">
                    <div className="w-14 h-14 rounded-full dark:bg-slate-800 bg-slate-100 border dark:border-slate-700 border-slate-200 flex items-center justify-center overflow-hidden shadow-inner shrink-0 text-slate-400">
                       <span className="text-sm font-bold">{appt.doctorName.split(' ').map(n => n[0]).join('').slice(0,3)}</span>
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="font-bold theme-text-primary text-lg">{appt.doctorName}</h3>
                        {appt.doctorIsVerified && (
                      ...
                      <p className="text-sm text-primary-400 font-bold mb-1">{appt.specialty}</p>
                      <p className="text-xs theme-text-muted flex items-center gap-1 font-medium">
                  ...
                  <div className="flex flex-col sm:items-end gap-2 w-full sm:w-auto">
                    <div className="dark:bg-white/10 bg-[#CBC5EA]/10 border dark:border-white/10 border-[#73628A]/10 theme-text-primary text-sm font-bold px-3 py-1.5 rounded-lg inline-flex items-center gap-2 w-fit">
                  ...
                  <div className="flex gap-3 w-full sm:w-auto mt-2 sm:mt-0">
                     <button onClick={() => setShowRescheduleModal(appt.id)} className="flex-1 sm:flex-none border dark:border-white/10 border-[#73628A]/10 theme-text-muted px-4 py-2 rounded-xl text-sm font-bold hover:bg-[#CBC5EA]/5 dark:hover:bg-white/5 transition-colors">Reschedule</button>
                     <button onClick={() => cancelAppointment(appt.id)} className="flex-1 sm:flex-none border border-red-500/20 text-red-500 dark:text-red-400 px-4 py-2 rounded-xl text-sm font-bold hover:bg-red-500/10 transition-colors">Cancel</button>
                     {appt.type === 'telemedicine' && (
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4" onClick={() => setShowUploadModal(false)}>
          <div className="glass-card rounded-[32px] p-8 max-w-md w-full animate-fade-in-up shadow-2xl" onClick={e => e.stopPropagation()}>
            <h2 className="text-2xl font-bold text-[#313D5A] dark:text-white mb-6">📁 Upload Document</h2>
            <div className="border-2 border-dashed dark:border-white/10 border-[#73628A]/10 rounded-2xl p-8 text-center mb-6 hover:border-primary-500/30 transition-colors bg-[#CBC5EA]/5 dark:bg-transparent">
              <p className="theme-text-muted mb-4 font-bold">Drag & drop or click to upload</p>
              <input type="text" placeholder="File name (e.g. Lab_Report.pdf)" className="w-full dark:bg-white/5 bg-white border dark:border-white/10 border-slate-200 rounded-xl px-4 py-3 theme-text-primary placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-primary-500/50 font-medium" value={uploadFileName} onChange={e => setUploadFileName(e.target.value)} />
            </div>
            <div className="mb-4">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowUploadModal(false)} className="flex-1 border dark:border-white/10 border-slate-200 theme-text-muted py-4 rounded-xl font-bold hover:bg-slate-100 dark:hover:bg-white/5 transition-all">Cancel</button>
              <button onClick={() => { if (uploadFileName.trim()) { addDocument(uploadFileName.trim()); setUploadFileName(''); setShowUploadModal(false); } }} className="flex-1 bg-primary-600 hover:bg-primary-500 text-white py-4 rounded-xl font-bold shadow-lg shadow-primary-500/30 transition-all">Upload</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showRescheduleModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4" onClick={() => setShowRescheduleModal(null)}>
          <div className="glass-card rounded-[32px] p-8 max-w-md w-full animate-fade-in-up shadow-2xl" onClick={e => e.stopPropagation()}>
            <h2 className="text-2xl font-bold theme-text-primary mb-6">📅 Reschedule</h2>
            <div className="space-y-4 mb-8">
              <div>
                <label className="text-xs font-bold theme-text-muted ml-1 block mb-2 uppercase tracking-wide">New Date</label>
                <select className="w-full dark:bg-white/5 bg-[#CBC5EA]/5 border dark:border-white/10 border-[#73628A]/10 rounded-xl px-4 py-4 theme-text-primary outline-none focus:ring-2 focus:ring-primary-500/50 font-medium" value={rescheduleDate} onChange={e => setRescheduleDate(e.target.value)}>
                  <option value="" className="dark:bg-slate-900 bg-white">Select a date</option>
                  {['Oct 27', 'Oct 28', 'Oct 29', 'Oct 30', 'Nov 1'].map(d => <option key={d} value={d} className="dark:bg-slate-900 bg-white">{d}</option>)}
                </select>
              </div>
              <div>
                <label className="text-xs font-bold theme-text-muted ml-1 block mb-2 uppercase tracking-wide">New Time</label>
                <select className="w-full dark:bg-white/5 bg-slate-50 border dark:border-white/10 border-slate-200 rounded-xl px-4 py-4 theme-text-primary outline-none focus:ring-2 focus:ring-primary-500/50 font-medium" value={rescheduleTime} onChange={e => setRescheduleTime(e.target.value)}>
                  <option value="" className="dark:bg-slate-900 bg-white">Select a time</option>
                  {['09:00 AM', '10:00 AM', '11:00 AM', '02:00 PM', '03:00 PM'].map(t => <option key={t} value={t} className="dark:bg-slate-900 bg-white">{t}</option>)}
                </select>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setShowRescheduleModal(null)} className="flex-1 border dark:border-white/10 border-slate-200 theme-text-muted py-4 rounded-xl font-bold hover:bg-slate-100 dark:hover:bg-white/5 transition-all">Cancel</button>
              <button onClick={() => { if (rescheduleDate && rescheduleTime) { rescheduleAppointment(showRescheduleModal, rescheduleDate, rescheduleTime); setShowRescheduleModal(null); setRescheduleDate(''); setRescheduleTime(''); } }} className="flex-1 bg-primary-600 hover:bg-primary-500 text-white py-4 rounded-xl font-bold shadow-lg shadow-primary-500/30 transition-all">Confirm</button>
            </div>
          </div>
        </div>
      )}
